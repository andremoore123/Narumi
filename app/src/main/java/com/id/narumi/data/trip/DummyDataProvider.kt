...
object DummyDataProvider {
    fun getTripResponses(): List<TripResponse> {
        ...
        return tripImages.indices.map { index ->
            TripResponse(
                imageResId = tripImages[index],
                name = tripNames[index],
                location = tripLocations[index],
                date = tripDates[index],
                orderDate = tripOrderDates[index],
                duration = tripDurations[index],
                cost = tripCosts[index],
                description = tripDescriptions[index],
                category = tripCategories[index]
            )
        }
    }
}